...
public final class PluginBundleManager
{
    ...
    public static boolean isBundleValid(final Bundle bundle)
    {
         ...
         */
        if (!bundle.containsKey(BUNDLE_EXTRA_STRING_PATTERN))
        {
            if (Constants.IS_LOGGABLE)
            {
                Log.e(Constants.LOG_TAG,
                      String.format("bundle must contain extra %s", BUNDLE_EXTRA_STRING_PATTERN)); //$NON-NLS-1$
            }
            return false;
        }
        if (!bundle.containsKey(BUNDLE_EXTRA_STRING_TITLE))
        {
            ...
            {
                Log.e(Constants.LOG_TAG,
                        String.format("bundle must contain extra %s", BUNDLE_EXTRA_STRING_TITLE)); //$NON-NLS-1$
            }
            return false;
        }
        ...
        if (TextUtils.isEmpty(bundle.getString(BUNDLE_EXTRA_STRING_PATTERN)))
        {
            if (Constants.IS_LOGGABLE)
            {
                Log.e(Constants.LOG_TAG,
                      String.format("bundle extra %s appears to be null or empty.  It must be a non-empty string", BUNDLE_EXTRA_STRING_PATTERN)); //$NON-NLS-1$
            }
            return false;
        }

        if (TextUtils.isEmpty(bundle.getString(BUNDLE_EXTRA_STRING_TITLE)))
        {
            if (Constants.IS_LOGGABLE)
            {
                Log.e(Constants.LOG_TAG,
                        String.format("bundle extra %s appears to be null or empty.  It must be a non-empty string", BUNDLE_EXTRA_STRING_TITLE)); //$NON-NLS-1$
            }
            return false;
        }
     ...
     * @param title The item to be displayed by the plug-in. Cannot be null.
     * @param pattern The vibration pattern to be fired.  Cannot be null.
     * @return A plug-in bundle.
     */
    public static Bundle generateBundle(final Context context, final String title, final String pattern)
    {
        ...
        result.putString(BUNDLE_EXTRA_STRING_PATTERN, pattern);
